...
#define LV_DRV_DELAY_MS(ms)  vTaskDelay(ms/portTICK_PERIOD_MS)       /*Delay the given number of milliseconds*/
...
#define LV_DRV_DISP_RST(val)        display_rst(val) /*pin_x_set(val)*/    /*Set the reset pin to 'val'*/

/*---------
 *  SPI
 *---------*/
#define LV_DRV_DISP_SPI_CS(val)          display_spi_cs(val) /*spi_cs_set(val)*/     /*Set the SPI's Chip select to 'val'*/
#define LV_DRV_DISP_SPI_WR_BYTE(data)    display_spi_wr_byte(data) /*spi_wr(data)*/        /*Write a byte the SPI bus*/
#define LV_DRV_DISP_SPI_WR_ARRAY(adr, n) display_spi_wr_array(adr, n) /*spi_wr_mem(adr, n)*/  /*Write 'n' bytes to SPI bus from 'adr'*/
